...
<!--
    ...
    Based on the original iso_schematron_skeleton_for_xslt1.xsl stylesheet
    with slight modifications by François "Clicky" Poirotte.

    Customizations:

    -   Change the way failures are reported so as to mimick reports
        generated by PHPUnit.

    -   Add the PHP XSL namespace to the validating stylesheet and use
        an helper method in Erebot\DOM to add line information in failure
        reports.
-->
