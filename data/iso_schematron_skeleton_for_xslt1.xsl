...
<xsl:template match="iso:schema[not(@queryBinding) or @queryBinding='xslt' 
	...
	<axsl:stylesheet>
	    ...
	    <xsl:apply-templates select="php:ns"/>
	    <!-- Handle the namespaces before the version attribute: reported to help SAXON -->
	    <xsl:attribute name="version">1.0</xsl:attribute>

	    <!-- @Clicky: add PHP as an extension. -->
	    <xsl:attribute name="xsl:extension-element-prefixes">php</xsl:attribute>
...
<xsl:template name="process-assert">
    ...
    -->
    <axsl:element name="assertionFailure">
        <axsl:attribute name="test"><xsl:value-of select="$test"/></axsl:attribute>

        <axsl:element name="location"><axsl:apply-templates select="." mode="schematron-get-full-path" /></axsl:element>
        <axsl:element name="description"><xsl:apply-templates mode="text"/></axsl:element>
        <axsl:element name="line"><axsl:value-of select="php:function('\Erebot\DOM::getNodeLineNo', .)"/></axsl:element>
    </axsl:element>
</xsl:template>
...
<xsl:template name="process-report">
    ...
    <fact test="{$test}">
        <axsl:element name="location"><axsl:apply-templates select="." mode="schematron-get-full-path" /></axsl:element>
        <axsl:element name="description"><xsl:apply-templates mode="text"/></axsl:element>
        <axsl:element name="line"></axsl:element>
    </fact>
</xsl:template>
